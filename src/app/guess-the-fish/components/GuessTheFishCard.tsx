...
export const GuessTheFishCard = ({ fish, listOfFish }: { fish: string, listOfFish: string[] }) => {
    ...
    const makeGuess = () => {
        if(!guessList.includes(textGuess) && textGuess !== '' && !reveal) {
            ...
            if(textGuess.toLowerCase() === fish.toLowerCase()) setCorrectGuess(true);
        };
    }
    ...
    return (
        <div className={`bg-zinc-900 ${reveal && (correctGuess ? 'outline outline-green-400 outline-1' : 'outline outline-red-600 outline-1')} drop-shadow-lg rounded-lg px-10 py-3 flex flex-col items-center text-lg gap-2 w-96`}>
            <div className='w-fit rounded-xl h-fit bg-zinc-700 flex flex-row items-center justify-center'>
            ...
            <div className={`${(reveal && !correctGuess) ? 'opacity-100 h0-fit' : 'opacity-0 h-0'} text-red-600 text-center`}>Go Fish. Better Luck Next Time</div>

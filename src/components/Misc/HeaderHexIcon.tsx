...
export const HeaderHexIcon = ({icon, name, onClick, isMobile, invert}: Props) => {
    ...
    return (
        <div className={`flex justify-center items-center overflow-clip select-none drop-shadow-md group cursor-pointer`} onClick={() => onClick()}>
            ...
            <Image src={icon} alt='icon' className={`absolute flex-shrink ${invert && 'invert'}`} height={isMobile ? 25 : 40 } width={isMobile ? 25 : 40}/>

...
export const HexIcon = ({icon, width, height, name}: Props) => {

    const textSize = width || 0 > 32 ? 'text-lg' : 'text-sm';
    const imgSize = width || 0 > 32 ? 'max-w-16 max-h-16' : 'max-w-12 max-h-12 p-2';

    return (
        <div className={`flex justify-center items-center overflow-clip select-none drop-shadow-md group`}>
            <svg className='w-16 h-16 fill-primary z-10' viewBox="0 0 56 56" xmlns="http://www.w3.org/2000/svg" 
            ...
            <Image src={icon} alt='icon' width={50} height={50} className={`absolute ${imgSize} flex-shrink`}/>
            <span className={`${textSize} absolute bottom-0 text-gray-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20`}>{name}</span>
        </div>
    )
};
